...
def extract_movie_details(movie_link):
    ...
    if movie_soup is not None:
        ...
        if movie_data is not None:
            ...
            role = movie_data.find_all('li', {'role': 'presentation'})

            date = role[2].get_text()[0:4] if '20' in role[2].get_text(
            ) or '19' in role[2].get_text()[0:4] else role[3].get_text()[0:4]

        rating = movie_soup.find('div', attrs={'data-testid': 'hero-rating-bar__aggregate-rating__score'}).get_text()[
            0:3] if movie_soup.find('div', attrs={'data-testid': 'hero-rating-bar__aggregate-rating__score'}).get_text()[0:3] else None

        plot_text = movie_soup.find_all('span', {'role': 'presentation'})[-1].get_text().strip(
        ) if movie_soup.find_all('span', {'role': 'presentation'})[-1].get_text().strip() else None
